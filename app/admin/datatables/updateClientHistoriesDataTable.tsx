...
import { Client, UpdateHistory } from "@/app/utils/interfaces";
import Image from "next/image";
import Link from "next/link";


interface UpdateClientHistoriesDataTableProps<TData, TValue> {
  data: TData[];
  onRemove: (data: UpdateHistory) => void;
}

export function UpdateClientHistoriesDataTable<TData extends UpdateHistory, TValue>({
  data = [],
  onRemove,
}: UpdateClientHistoriesDataTableProps<TData, TValue>) {
  ...
  const columns = useMemo<ColumnDef<UpdateHistory, unknown>[]>(
    () => [
      // {
      //   id: "actions",
      //   header: "Actions",
      ...
      { accessorKey: "client_id", header: "Page Id", cell: ({ row }) =>{
        return <Link href={`/${row.original.client_id}`} target="_blank" className="underlined hover:underline hover:text-blue-500">{row.original.client_id}</Link>;
      }},
      ...
      { accessorKey: "address", header: "Address", cell: ({ row }) =>{
        return <div className="underlined">{row.original.address}</div>;
      }},
      { accessorKey: "gmap", header: "Map Location" },
      { accessorKey: "full_description", header: "Full Description" },
      {
        accessorKey: "images",
        header: "Images",
      },
  ...
  return (
    <div className="container px-auto overflow-x-auto ">
      <div className="flex items-center justify-between py-4">
        <h2 className="text-xl text-black font-semibold">Updated Pages</h2>
