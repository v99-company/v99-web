...
export default function LoginPage() {
  ...
  const loginUser = async (user: any) => {
    try {
      const response = await fetch('/api/freelancerLogin', {
        method: 'POST',
      ...
      if (response.ok) { 
        ...
        localStorage.setItem('freelancerLoginToken', data.token);
        ...
        console.error('Freelancer Login failed:', errorData.message || 'An error occurred');
  ...
  return (
    <div className="w-screen h-screen bg-gray-300 flex items-center justify-center">
      <div className="w-[400px] bg-white rounded-lg shadow-lg p-8">
        <h2 className="text-center text-2xl font-semibold mb-6">Freelancer Log In</h2>
