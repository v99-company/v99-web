...
export async function PUT(req: NextRequest) {
  try {
    ...
    if (!response.ok) {
        ...
        console.error("Error from update admin service:", errorText);
        return NextResponse.json({ message: 'admin failed to be updated' }, { status: 404 });
    }
...
export async function DELETE(req: NextRequest) {
  try {
    const pathParts = req.nextUrl.pathname.split('/');
    const id = pathParts.pop();
    const response = await fetch(`${baseUrl}/block_admin.php?id=${id}`);
